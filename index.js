...
function initGoogleSignIn() {
  if (window.google && state.activeTab === 'settings') {
    try {
      ...
      console.warn("Google GSI not available or configured.");
    }
  }
}
    ...
    <div class="h-screen flex flex-col overflow-hidden max-w-lg mx-auto border-x ${state.isDarkMode ? 'border-slate-800' : 'border-gray-200'}">
      ...
      <nav class="fixed bottom-0 left-0 right-0 max-w-lg mx-auto bg-inherit border-t p-4 flex justify-around items-center z-50 ${state.isDarkMode ? 'border-slate-800 bg-slate-950' : 'border-gray-100 shadow-lg bg-white'}">
        ${['home', 'meds', 'emergency', 'settings'].map(tab => {
          const icon = ICONS[tab === 'meds' ? 'meds' : tab === 'emergency' ? 'emergency' : tab];
          return `
          <button onclick="setState({activeTab: '${tab}'})" class="flex flex-col items-center gap-1 ${state.activeTab === tab ? 'text-blue-600 scale-110' : 'text-gray-400 opacity-60'} transition-all">
            <div class="p-2 rounded-2xl ${state.activeTab === tab ? 'bg-blue-50' : ''}">${icon}</div>
  ...
  if (state.activeTab === 'meds') return `
    <div class="space-y-6">
      ...
      <div class="space-y-4">
        ...
        `).join('')}
        ${(state.medicines || []).length === 0 ? '<p class="text-center py-10 opacity-30 font-bold">No medicines added yet.</p>' : ''}
      </div>
    </div>
  `;
    ...
    <div class="space-y-8 animate-in slide-in-from-bottom-10">
      ...
      <div class="bg-white ${state.isDarkMode ? 'bg-slate-900 border-slate-800' : 'border-gray-100'} border-2 rounded-[40px] overflow-hidden">
        ...
        <div class="p-8 flex items-center justify-between border-b ${state.isDarkMode ? 'border-slate-800' : 'border-gray-100'}">
          ...
          <button onclick="window.requestNotificationPermission()" class="w-14 h-8 rounded-full ${state.notificationsEnabled ? 'bg-green-500' : 'bg-gray-200'} p-1 transition-all">
            <div class="w-6 h-6 bg-white rounded-full shadow-md transition-all ${state.notificationsEnabled ? 'translate-x-6' : ''}"></div>
          </button>
        </div>

        <div class="p-8 flex items-center justify-between border-b ${state.isDarkMode ? 'border-slate-800' : 'border-gray-100'}">
          <div><h4 class="text-xl font-black">Dark Mode</h4><p class="text-sm opacity-50 font-bold">Easier for reading at night</p></div>
          <button onclick="setState({isDarkMode: !state.isDarkMode})" class="w-14 h-8 rounded-full ${state.isDarkMode ? 'bg-blue-600' : 'bg-gray-200'} p-1 transition-all">
            <div class="w-6 h-6 bg-white rounded-full shadow-md transition-all ${state.isDarkMode ? 'translate-x-6' : ''}"></div>
          </button>
        </div>
        ...
        <button onclick="window.resetApp()" class="w-full p-8 text-left hover:bg-red-50 transition-colors">
          ...
          <p class="text-sm opacity-50 font-bold">Clear data and restart intro</p>
        </button>
      </div>
      <div class="text-center opacity-30 font-black text-[10px] uppercase tracking-[0.3em] pb-20">Senior Assist v1.7 • Stable Icons</div>
    </div>
  `;
}
...
window.resetApp = () => {
  if(confirm('This will clear all data and show the intro again. Continue?')) {
    localStorage.clear();
    location.reload();
  }
};
